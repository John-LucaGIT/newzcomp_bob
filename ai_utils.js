...
// Model configuration
const GPT_MODEL = 'gpt-4.1'; // GPT-4.1 - Smartest non-reasoning model with 1M token context
...
async function generateSearchQueryFromArticle({ title, description }) {
  const prompt = `You are a news search expert.
...
async function extractArticleConcepts(articleInfo) {
  const prompt = `You are a news analysis expert. Your task is to deconstruct a news article into its core components for a search query.
...
async function findRelevantArticleWithAI(linkCandidates, searchQuery) {
  ...
  const prompt = `You are a news article selector. Given a search query and a list of article links with their titles and context, select the MOST RELEVANT article that relates to the search query.
...
async function analyzeArticlesWithAI(articles) {
  const payload = articles.map((a) => ({
    source: a.source,
    title: a.title,
    text: a.text.slice(0, 100_000), // GPT-4.1 has 1M token context window
  }));

  const prompt = `Your name is Bob, you are a professional news analyst trained to detect bias.
...
async function analyzeArticlesWithAINew(articles) {
  ...
  console.log(
    'Payload for OpenAI:',
    payload.map((p) => ({ source: p.source, title: p.title, textLength: p.text.length }))
  );
...
Note we also add a summary of each article in the bias array. Here is an example of the output format:
{
  ...
  "conclusion": "The collective narrative from the articles presents...",
